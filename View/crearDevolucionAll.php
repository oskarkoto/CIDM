...
<?php   }  ?>
<!-- Vista de Dispositivos de Prestamo. -->
<div class="upside">
...
<div class="card" id="ListaCompleta">
    ...
    <table class="table">
        <thead>
            <tr>
                <th scope="col">ID</th>
                <th scope="col">Dispositivo/Suministro</th>
                <th scope="col">Condición en Inventario</th>
                <th scope="col">Editar</th>
            </tr>
        </thead>
        <tbody>
                <?php foreach ($allPrestamoDispositivo as $prestamoE) { if($Prestamo == $prestamoE->idPrestamo)  {
                  foreach ($allDispositivo as $Dispositivo)  {  if ($prestamoE->idDispositivo == $Dispositivo->idDispositivo ){
                  foreach($allTipoDispositivo as $tipo) {  if($Dispositivo->idTipoDispositivo == $tipo->idTipoDispositivo) { ?>
            <tr>
            
                   <td><?php echo $Dispositivo->idDispositivo; ?></td>
                   <td><?php echo $tipo->nombreTipoDispositivo; ?></td>
                <?php  } }  ?> 
                <?php foreach ($allCondicionActual as $estado1) { if ($estado1->idCondicionActual == $Dispositivo->idCondicionActual) {?>
                <td><?php echo $estado1->descripcionCondicionActual	; ?></td>
                <?php  } } ?>
                <td>
                <br>
                <a class="editar" id="editar" href="?c=actualizarCondDispositivo&idDispositivo=<?php echo $Dispositivo->idDispositivo; ?>">
                <!-- Icono Actualizar -->
